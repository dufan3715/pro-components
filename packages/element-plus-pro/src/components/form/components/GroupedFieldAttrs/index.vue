 ...
 * 3. **componentProps** — 组件属性（placeholder、clearable 等），传递给输入组件
 * 4. **formItemSlots** — FormItem 的具名插槽（label、error 等）
 *
 * 分组依据：
 * - 匹配 element-plus GridItem 的 props key → gridItemProps
 * - 匹配 element-plus FormItem 的 props key → formItemProps
...
const groupedAttributes = computed<GroupedFieldAttrs>(() => {
  ...
  if (isPlainObject(props.field)) {
    ...
    const initGridItemProps =
      span || span === 0 ? { span } : { xs, sm, md, lg, xl, span: injectSpan };
