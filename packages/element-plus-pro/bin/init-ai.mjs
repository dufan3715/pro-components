...
function getSkillContent() {
  ...
  sections.push(
    `description: ${PKG_NAME} 完整 API 参考 - 组件、Hooks、类型、Field/Column 配置。当需要精确的函数签名、参数类型或完整用法示例时调用。`
  );
  sections.push('---');
  sections.push('');
...
function main() {
  ...
  console.log(`   - 编写 Field 中输入控件属性前，查阅 element-plus 官方文档：${cyan('https://element-plus.org')}`);
  console.log('');
}
