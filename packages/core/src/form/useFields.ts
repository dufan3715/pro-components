...
export type FR<F, FormInstance> = Readonly<F & AM<FormInstance>>;
...
> {
   ...
   * @returns {FR<F, FormInstance> | undefined} 匹配到的字段配置（只读），未找到返回 undefined
   *
   * @example
   * ```ts
   * // 通过路径
   ...
   * ```
   */
  getField(path: Path<D> | FieldFindBy<D, F>): FR<F, FormInstance> | undefined;
  /**
  ...
  getField(
    ...
    options?: { all?: false }
  ): FR<F, FormInstance> | undefined;
  /**
   * 根据路径或查找函数获取所有匹配的字段配置
   *
   * @param path - 字段路径或查找函数
   * @param options - 选项，{ all: true } 表示返回所有匹配
   * @returns {FR<F, FormInstance>[] | undefined} 所有匹配的字段配置数组
   *
   * @example
   * ```ts
   ...
   * @returns {FR<F, FormInstance> | undefined} 父级字段配置（只读），如果是一级字段则返回虚拟根容器
   *
   * @example
   * ```ts
...
>(
  initFields?: F[]
): UseFieldsReturn<D, F, FormInstance> => {
  ...
  type R = UseFieldsReturn<D, F, FormInstance>;
  ...
  return {
    ...
    appendField,
    prependField,
    getParentField: getParentField as R['getParentField'],
  };
};
