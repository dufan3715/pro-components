...
import '@qin-ui/antd-vue-pro/antd-vue-pro.css';
app.use(AntdVuePro); // 全局注册 ProForm / ProTable / ProComponentProvider
```
...
返回 `Form` 实例（**2.0+ `formData` 是 reactive 对象，不用 `.value`**）：
...
**关键：ProTable 透传所有 ant-design-vue TableProps**（`& TableProps`），可直接写 `row-key` / `bordered` / `scroll` / `row-selection` 等原生属性；`size`、`loading` 是 v-model。
...
继承 ant-design-vue `ColumnType`，**所有原生列属性可用**（`title` / `width` / `fixed` / `align` / `customRender` / `ellipsis`…）。新增：
...
- `node_modules/@qin-ui/antd-vue-pro/api.json` -- 结构化 API 元数据（函数签名、参数、类型）。
